...
const ChatReplyBar = ({replyTo, currentUserId, onCancel}: Props) => {
    ...
    return(
        <div className="absolute bottom-16 left-0 right-0 bg-card border-top animate-in slide-in-from-bottom pb-4 px-6">
            <div className="flex flex-1 justify-between mt-2 p-3 text-sm border-l-4 border-l-primary bg-primary/10 rounded-md shadow-sm">
                ...
                <Button variant="ghost" size="icon" onClick={onCancel} className="shrink-0 size-6">
                    <X size={14} />
                </Button>
            </div>
        </div>

...
export const ChatMessageBody = memo(({message, onReply}: Props) => {
    const {user} = useAuth();
    ...
    const messageClass = cn("min-w-[200px] px-3 py-2 text-sm break-words shadow-sm", isCurrentUser ? "bg-accent dark:bg-primary/40 rounded-l-xl rounded-tr-xl" : "bg-[#F5F5F5] dark:bg-accent rounded-bl-xl rounded-r-xl")
    const replyBoxClass = cn(`mb-2 p-2 text-xs rounded-md border-l-4 shadow-md !text-left`, isCurrentUser ? "bg-primary/20 border-l-primary" : "bg-gray-200 dark:bg-secondary border-l-[#CC4A31]")
